    ...
    <header className="sticky top-0 z-50 py-6 font-sans shadow-lg bg-gradient-to-r from-[#F5F5F5]/50 to-[#D6D3CD]/50 dark:from-gray-900/50 dark:to-gray-800/50 text-[#222222] dark:text-white backdrop-blur-md">
      <div className="container px-4 mx-auto">
        <div className="flex items-center justify-between">
          ...
          {/* Menu burger mobile */}
          <button className="p-2 rounded md:hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={menuOpen ? 'Fermer le menu' : 'Ouvrir le menu'} onClick={() => setMenuOpen(!menuOpen)}>
            {menuOpen ? <FaTimes className="w-6 h-6" /> : <FaBars className="w-6 h-6" />}
          </button>
        </div>
        {/* Menu mobile */}
        {menuOpen && (
          <nav className="flex flex-col items-center py-4 mt-4 space-y-4 bg-white rounded shadow-lg md:hidden dark:bg-gray-900" aria-label="Navigation mobile">
